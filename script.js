...
const appendScript = (code, isModule = false) => {
...
const normalizedPage = currentPage || 'index.html';
switch (normalizedPage) {
  case 'about.html':
    appendScript(`const io = new IntersectionObserver((entries)=> {
...
const menu = document.getElementById('mobileMenu');
if (btn && menu) btn.addEventListener('click', () => menu.classList.toggle('hidden'));`);
    appendScript(`import { initializeApp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js";
...
setupSocialLogin("microsoftBtn", () => new OAuthProvider("microsoft.com"), "Microsoft");`, true);
    break;
  case 'conseil.html':
    appendScript(`import { initializeApp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js";
...
onAuthStateChanged(auth, (user)=>{
  ...
  if(!user){
    ...
    break;
  case 'firebase.html':
    appendScript(`document.addEventListener('DOMContentLoaded', function() {
...
document.querySelectorAll('.reveal').forEach(el => io.observe(el));`);
    appendScript(`// Toggle menu mobile
const btn = document.getElementById('menuBtn');
const menu = document.getElementById('mobileMenu');
if (btn && menu) {
  btn.addEventListener('click', () => {
    ...
    appendScript(`import { initializeApp, getApps } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js";
import { getAuth, onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js";
...
setInterval(() => {
  ...
  refreshInfofootBadge();
    ...
    appendScript(`// Met à jour le badge "CONSEIL DU JOUR" à partir du localStorage
(function(){
...
onAuthStateChanged(auth, (user)=>{
  ...
  case 'inscription.html':
    appendScript(`    import { initializeApp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js";
    import { getAuth, createUserWithEmailAndPassword, GoogleAuthProvider, FacebookAuthProvider, OAuthProvider, signInWithPopup, RecaptchaVerifier, signInWithPhoneNumber, updateProfile } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js";
...
loadNotifications();`, true);
    appendScript(`import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
...
onAuthStateChanged(auth, (user)=>{
  ...
  wireAuthGuards(user);
    ...
    break;
  case 'page payment.html':
    appendScript(`// ---- Helpers
...
const btn = document.getElementById('menuBtn');
const menu = document.getElementById('mobileMenu');
if (btn && menu) btn.addEventListener('click', () => menu.classList.toggle('hidden'));`, true);
    appendScript(`import { initializeApp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js";
import { getAuth, onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js";
...
onAuthStateChanged(auth, (user)=>{
  ...
  case 'profil.html':
    appendScript(`import { initializeApp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js";
import {
  getAuth, onAuthStateChanged, signOut, updateEmail, updateProfile,
  sendEmailVerification, sendPasswordResetEmail, deleteUser, reauthenticateWithPopup,
  GoogleAuthProvider
...
deleteAccountBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    break;
  case 'victoires.html':
    appendScript(`// ----- Firebase (même projet que tes autres pages) -----
