...
function cardTemplate(d) {
  ...
  const excerpt =
    (d.body || "").slice(0, 140) + ((d.body || "").length > 140 ? "…" : "");
  ...
  return `
        <article class="glass rounded-2xl p-3 card-anim fade-in">
          <div class="relative">
            ${
              img
                ? `<img src="${img}" alt="" class="w-full h-44 object-cover rounded-xl mb-3">`
...
function updateUnreadBadgeState() {
  ...
  const count = allDocs.filter((d) => {
    const ms = d.publishedAt?.toMillis
      ? d.publishedAt.toMillis()
      : new Date(d.publishedAt).getTime();
    return ms > lastRead;
  }).length;
...
$markAll.addEventListener("click", () => {
  ...
  setTimeout(() => {
    $markAll.textContent = "Marquer tout comme lu";
  }, 1500);
});
